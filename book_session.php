...
<?php
    ...
    if ($result->num_rows > 0) {
        ...
        if ($mentee_sport_level <= $sport_max_level){
            $mentee = "UPDATE `skill` SET `sport_level`='$mentee_sport_level' WHERE `user_id` = $mentee_id AND `sport_id` = $sport_id";
            $mentee_result = $conn->query($mentee);
        }
        
        if ($mentor_sport_level <= $sport_max_level){
            $mentor = "UPDATE `skill` SET `sport_level`='$mentor_sport_level' WHERE `user_id` = $mentor_id AND `sport_id` = $sport_id";
